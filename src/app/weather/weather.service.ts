...
export class WeatherService {
	constructor(private http: HttpClient, private timeFormatService: TimeFormatService) {}
	...
	private getWeatherAsync(coordinates: Coordinates): Promise<Weather[]> {
		let url = `https://opendata-download-metfcst.smhi.se/api/category/pmp3g/version/2/geotype/point/lon/${coordinates.lon.toFixed(6)}/lat/${coordinates.lat.toFixed(6)}/data.json`;
		return this.http
			.get(url)
			.toPromise()
			.then(response => {
				let weather = this.getWeatherData(response);
				return weather;
			})
			.catch(this.useTestData);
	}
	...
	private useTestData(error: any): Promise<Weather[]> {
		return Promise.resolve(SampleData.data);
	}
}
